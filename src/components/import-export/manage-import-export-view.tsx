    ...
    <section className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          ...
          <div className="rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-800/50">
            <input
              name="file"
              required
              type="file"
              accept=".html,.htm,.csv,.json"
              className="block w-full rounded-md border border-slate-300 bg-white px-2 py-2 text-sm text-slate-700 file:mr-3 file:rounded-md file:border-0 file:bg-slate-900 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-white hover:file:bg-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:file:bg-slate-700 dark:hover:file:bg-slate-600"
            />
            <div className="mt-3 flex flex-wrap gap-2">
              {importFormats.map((item) => (
                <span
                  key={item}
                  className="rounded border border-slate-200 bg-white px-2 py-1 text-xs text-slate-600 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-400"
                >
                  {item}
                </span>
              ))}
            ...
            {exportFormats.map((item) => {
              ...
              return (
                <Link
                  ...
                  className="inline-flex w-full items-center justify-between rounded border border-slate-300 px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800"
                >
