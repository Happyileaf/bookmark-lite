...
export function UserMenu({ email, isAdmin, userLabel }: UserMenuProps) {
  ...
  return (
    <details ref={detailsRef} className="relative z-50">
      <summary className="flex cursor-pointer list-none items-center p-0 text-slate-700 dark:text-slate-300">
      ...
      <div className="absolute right-0 z-50 mt-2 w-60 rounded border border-slate-200 bg-white p-2.5 shadow-lg dark:border-slate-700 dark:bg-slate-800">
        <div className="space-y-2 border-b border-slate-100 px-2.5 pt-1.5 pb-4 dark:border-slate-700">
          ...
          <div className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs leading-4 text-slate-600 dark:bg-slate-700 dark:text-slate-400">
            {isAdmin ? "超级管理员" : "普通用户"}
          </div>
        </div>
        ...
        <div className="mt-2.5 text-sm">
          <div className="grid gap-1 border-b border-slate-100 px-1 pb-2 dark:border-slate-700">
            <Link
              href="/my-bookmarks"
              onClick={closeMenu}
              className="rounded px-2 py-1.5 text-slate-700 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-700"
            >
              个人空间
            </Link>
            <Link
