...
function DatabaseUnavailableNotice({ reason, runtimeTarget }: { reason: string; runtimeTarget: RuntimeTarget }) {
  ...
  return (
    <section className="rounded border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-200">
      ...
      {isVercel ? (
        <p className="mt-2">
          ...
          <code className="mx-1 rounded bg-amber-100 px-1 py-0.5 dark:bg-amber-900 dark:text-amber-200">DATABASE_URL</code>
          后重新部署；首次可执行
          <code className="mx-1 rounded bg-amber-100 px-1 py-0.5 dark:bg-amber-900 dark:text-amber-200">npm run db:migrate:deploy</code>
          初始化表结构。
        </p>
      ) : (
        <p className="mt-2">
          请先准备
          <code className="mx-1 rounded bg-amber-100 px-1 py-0.5 dark:bg-amber-900 dark:text-amber-200">.env</code>
          并配置
          <code className="mx-1 rounded bg-amber-100 px-1 py-0.5 dark:bg-amber-900 dark:text-amber-200">DATABASE_URL</code>
          ，确保 PostgreSQL 在本地 5432 端口可访问；首次可执行
          <code className="mx-1 rounded bg-amber-100 px-1 py-0.5 dark:bg-amber-900 dark:text-amber-200">npm run db:setup</code>
          初始化表结构。
        </p>
      )}
    </section>
...
export async function DisplayBookmarksView({ scope, user, searchParams }: Props) {
  ...
  return (
    <section className="grid h-full min-h-0 overflow-hidden md:grid-cols-[240px_minmax(0,1fr)]">
      <aside className="flex h-full min-h-0 self-stretch flex-col overflow-y-auto overflow-x-hidden rounded-b border-x border-b border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="min-h-0 min-w-0 space-y-4 overflow-y-auto p-6">
        <form className="flex items-center gap-2 rounded border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800">
          <input
            name="q"
            defaultValue={q}
            placeholder="搜索标题 / URL / 描述 / 标签"
            className="min-w-0 flex-1 rounded border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
          />
          {tagId ? <input type="hidden" name="tagId" value={tagId} /> : null}
          {!tagId ? <input type="hidden" name="view" value={view} /> : null}
          <button
            type="submit"
            className="shrink-0 whitespace-nowrap rounded bg-slate-900 px-4 py-2.5 text-sm text-white hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200"
          >
            搜索
          </button>
        </form>
