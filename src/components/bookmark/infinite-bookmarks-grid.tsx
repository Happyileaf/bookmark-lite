...
export function InfiniteBookmarksGrid({
  ...
  if (items.length === 0) {
    return (
      <div className="rounded border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-400">
        当前视图下暂无书签
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <ul className="grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 [@media(min-width:1920px)]:grid-cols-5">
        {items.map((bookmark) => (
          <li
            key={bookmark.id}
            className="group relative h-full rounded border border-slate-200 bg-white p-4 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-600"
          >
            <a
              href={bookmark.url}
              target="_blank"
            ...
            <div className="pointer-events-none relative z-20 grid h-full content-start gap-2">
              ...
              <p
                className="h-10 overflow-hidden text-sm leading-5 text-slate-700 dark:text-slate-300 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]"
              ...
              <div className="h-12 overflow-hidden" title={bookmark.tags.map((tag) => tag.name).join(" / ")}>
                <div className="flex flex-wrap gap-1">
                  ...
                  {bookmark.tags.length > 6 ? (
                    <span className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300">...</span>
                  ) : null}
                </div>
              </div>
            </div>
      ...
      <div className="space-y-2 rounded border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
